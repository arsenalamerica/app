...
- **Client-island scroll effect** *(revised in #65)*. `FixtureCardAnchor` was rewritten as a wrapper component that calls `scrollIntoView({ behavior: 'instant', block: 'center' })` on mount, centering the next fixture in the viewport without a DOM id. The `#next-fixture` anchor id was removed. DOM order is settled-first (oldest result → most recent settled → next upcoming → scheduled ahead), so the wrapper positions the browser at the correct card without URL-hash dependency.
...
- **Lighthouse performance on `/fixtures` meets the app-wide threshold.** List windowing (#65) reduced the initial HTML to ~10 real cards plus lightweight skeletons, clearing the `0.95` performance threshold previously unachievable with 55+ full cards streaming on load. The `assertMatrix` exemption in `lighthouserc.json` (renamed to `lighthouserc.js` in #294) was removed in #65.
